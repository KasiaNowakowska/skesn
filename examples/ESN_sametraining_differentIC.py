...
for I in range(len(IC)):
    ...
    for i in range(ensembles):
        ...
        '''
        #plot prediction for this IC initialisation
    ...
    plt.legend()
    figK.savefig(output_path+'/median_conf_int%i.png' % index_init)
    figA.savefig(output_path+'/timeseries_ensembles%i.png' % index_init)
...
axF[0].boxplot(ke_forecast.T, positions=forecast_times, labels=forecast_times)
axF[1].boxplot(q_forecast.T, positions=forecast_times, labels=forecast_times)
axF[0].plot(prediction_times, inverse_test_data[:,0])
axF[1].plot(prediction_times, inverse_test_data[:,1])
axF[0].set_ylabel('KE')
axF[1].set_ylabel('q')
axF[1].set_xlabel('time')
axF[0].set_xlim(forecast_times[0], forecast_times[-1])
axF[1].set_xlim(forecast_times[0], forecast_times[-1])
