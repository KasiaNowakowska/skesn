...
N_check      = 10   #each N_check epochs we check convergence and validation loss
...
checkpoint = tf.train.Checkpoint(optimizer=optimizer, enc_mod=enc_mods, dec_mod=dec_mods)
checkpoint_path = output_path + 'chkpt'
for epoch in range(n_epochs):
    ...
    if (epoch%N_check==0):
        ...
        if epoch > N_lr and lrate_update:
            ...
            if tt_loss > old_loss[epoch-N_lr]:
                ...
                optimizer.learning_rate.assign(optimizer.learning_rate.numpy() * lrate_mult)
    ...
    if (epoch%20==0) and epoch != 0:
        ...
        ax.legend()
        fig.savefig(output_path+'/Images'+'/convergence.png')
...
checkpoint.restore(tf.train.latest_checkpoint(output_path)).assert_consumed()
