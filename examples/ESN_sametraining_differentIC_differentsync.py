...
for I in range(len(IC)):
    index_init = IC0 + IC[I] - synclen  

    future_times = time_vals[index_init:index_init+synclen+predictionlen]
    sync_times = time_vals[index_init:index_init+synclen]
    prediction_times = time_vals[index_init+synclen:index_init+synclen+predictionlen]
    test_data = data[index_init+synclen:index_init+synclen+predictionlen, :]
    future_data = data[index_init:index_init+synclen+predictionlen, :]
    sync_data = data[index_init:index_init+synclen]


    ensemble_prediction_times[:,I] = prediction_times[:]

    print('initial start time before sync:', time_vals[index_init])
